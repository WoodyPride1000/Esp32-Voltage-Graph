...
消費電力を抑えるため、WiFiのモデムスリープ（間欠受信）を利用します。CPUは常時稼働のままWebサーバーは即時応答可能で、WiFi受信動作のみを間欠化することで消費電力を抑えます。Wi-Fi接続は非同期で管理され、接続状態をLEDで通知します。
...
* このシステムは、WiFiのモデムスリープを有効化することで消費電力を抑えています。CPU自体は停止しないため、サンプリング周期中もWebサーバーは常時応答可能です。
